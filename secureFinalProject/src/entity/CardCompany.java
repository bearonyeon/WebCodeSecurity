...
public class CardCompany {
	...
	public boolean verifyPayment(CardData cardData, PublicKey userPublicKey) throws CardCompanyException{
	    ...
	    try {
	        ...
	        paymentInfoHash = HashUtil.hashSalt(paymentInfoBytes, salt);
			...
			// 최종 해시 값 해시
			combinedHash = HashUtil.hash(combined); 
	        ...
	        throw new CardCompanyException("결제 정보 이중서명 검증 중 오류가 발생했습니다.", e);
	    		...
	    		// 메모리에 남아 있는 민감 데이터 제거
	        if (paymentInfoBytes != null) {
	        		Arrays.fill(paymentInfoBytes, (byte) 0);
	        }
	        if (paymentInfoHash != null) {
	        		Arrays.fill(paymentInfoHash, (byte) 0);
	        }
	        if (combined != null) {
	        		Arrays.fill(combined, (byte) 0);
	        }
	        if (combinedHash != null) {
	        		Arrays.fill(combinedHash, (byte) 0);
	        }
